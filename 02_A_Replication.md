...
| **Replication Slot**          | A replication slot is a persistent object on the primary server that tracks the state of replication by storing a pointer to the Log Sequence Number (LSN), indicating where the replica should resume reading from the WAL. It ensures that WAL logs are retained until they are no longer needed by the replica, preventing premature deletion. Since replication slots need to track and store this state for each replica, they consume system resources, including memory and file descriptors, to maintain replication integrity..                                | A **bookmark** placed at a specific line in the book.                                                               | **Prevents WAL bloat** by ensuring the required WAL is retained.                              |
